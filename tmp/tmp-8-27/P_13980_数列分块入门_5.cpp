...
void solve() {
	...
	int L = sqrt(n), B = ceil(1.0 *  (n - 1) / L) + 1;
	vector<bool> c(B, false);
	vector<i64> sum(B, 0);

	auto check = [&](int x) {
		bool f = true;
		for (int i = x * L; i < x * L + L && i < n; i++) {
			if (a[i] != 0 && a[i] != 1) {
				f = false;
			}
		}
	...
	for (int i = 0; i < n; i++) {
		...
		if (op == 0) {
			if (f(l) == f(r)) {
				...
				for (int j = l; j < getr(f(l)); j++) {
					sum[f(j)] -= a[j];
					a[j] = sqrt(a[j]);
					sum[f(j)] += a[j];
				}

				for (int j = f(l) + 1; j < f(r); j++) {
					if (c[j]) continue;
					for (int k = j * L; k < getr(j); k++) {
						sum[f(k)] -= a[k];
						a[k] = sqrt(a[k]);
						sum[f(k)] += a[k];
					}
