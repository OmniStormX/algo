...
struct ACAutomaton {
    ...
    int query(const string& text) {
        ...
        for (char c : text) {
            ...
            for (int j = u; j && trie[j].cnt != -1; j = trie[j].fail) {
                ans += trie[j].cnt;
                trie[j].cnt = -1; 
            }
        }
...
void solve() {
    ...
    matrix mat(L, L);
    for (int i = 0; i < L; i++) {
        for (int j = 0; j < ac.SIGMA; j++) {
            if (!ac.trie[ac.trie[i].nxt[j]].cnt) {
                mat.set(i, ac.trie[i].nxt[j], mat.get(i, ac.trie[i].nxt[j]) + 1);
            }
        }
    }
    // mat.print();
